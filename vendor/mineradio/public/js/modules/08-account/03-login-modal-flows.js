...
function selectLoginProviderNode(provider) {
  ...
  // In Abworkbench iframe embed (no Mineradio desktop preload), drag-to-wire is easy to miss.
  // Clicking a provider should also open the auth drawer for that source.
  if (!hasLoginWorkflowConnection(provider) && loginWorkflowPendingProvider !== provider) {
  ...
  if (copy) {
    ...
    if (copySub) copySub.textContent = hasLoginWorkflowConnection(loginProvider)
      ...
      : (loginWorkflowPendingProvider === loginProvider
        ...
        : (connectedCount
          ? ('已接入 ' + connectedCount + ' 个接口，点击扫码/Cookie 或拖入当前接口')
          : ((window.desktopWindow && window.desktopWindow.isDesktop)
            ? '把左侧接口拖入这里'
            : '点击左侧平台，或点扫码 / Cookie 开始登录')));
  }
...
function selectLoginMode(mode) {
  ...
  // Mode nodes used to only highlight state; users expect a click to start login.
  // If the current provider is not wired into MR yet, auto-connect then run the flow.
  if (!hasLoginWorkflowConnection(loginProvider) && loginWorkflowPendingProvider !== loginProvider) {
    connectLoginProviderToMr(loginProvider);
    return;
  }
  connectLoginMode(mode === 'cookie' ? 'cookie' : 'official');
}
...
async function openNeteaseWebLogin() {
  ...
  if (!api || !api.isDesktop || typeof api.openNeteaseMusicLogin !== 'function') {
    if (statusEl) {
      statusEl.textContent = '嵌入模式无官方登录窗，已切换为网易云 App 扫码二维码…';
      statusEl.className = 'preview';
    }
    showToast('嵌入模式：使用网易云 App 扫码');
    return refreshQr();
  }
...
async function openQQWebLogin() {
  ...
  if (!api || !api.isDesktop || typeof api.openQQMusicLogin !== 'function') {
    ...
    updateLoginProviderUi();
    if (statusEl) {
      statusEl.textContent = '当前嵌入环境无法弹出 QQ 官方登录窗，请改用 Cookie 粘贴导入（或单独运行 Mineradio 桌面版扫码）。';
      statusEl.className = 'fail';
    }
    showToast('嵌入模式请用 Cookie 登录 QQ');
    return;
  }
...
async function openKugouWebLogin() {
  ...
  if (!api || !api.isDesktop || typeof api.openKugouMusicLogin !== 'function') {
    ...
    updateLoginProviderUi();
    if (statusEl) {
      statusEl.textContent = '当前嵌入环境无法弹出酷狗官方登录窗，请改用 Cookie 粘贴导入（或单独运行 Mineradio 桌面版登录）。';
      statusEl.className = 'fail';
    }
    showToast('嵌入模式请用 Cookie 登录酷狗');
    return;
  }
