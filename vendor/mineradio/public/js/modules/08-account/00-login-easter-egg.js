...
async function ensureLoginEasterEggStatus(force) {
  // Abworkbench embed: never block login behind the easter-egg gate.
  var embedded = false;
  try {
    embedded = document.documentElement.classList.contains('abwb-embedded')
      || new URLSearchParams(location.search || '').get('embedded') === '1';
  } catch (_) {}
  if (embedded ||
      (window.desktopWindow && (window.desktopWindow.__abwbBridge || window.desktopWindow.__abwbNative))) {
