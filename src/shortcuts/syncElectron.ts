...
/**
 * Keep Electron global shortcuts (launcher / main window / quick capture) in sync with the renderer shortcut store.
 */
export function useSyncElectronShortcuts() {
  ...
  const push = useCallback(async (hotkey: string, mainWindowHotkey: string, quickCaptureHotkey: string) => {
  ...
  useEffect(() => {
    ...
    void push(launcher, mainWindow, quickCapture)
  }, [launcher, mainWindow, push, quickCapture])
}
