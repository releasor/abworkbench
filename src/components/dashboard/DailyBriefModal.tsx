...
export default function DailyBriefModal({
  ...
  return (
    <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/55 p-4 backdrop-blur-sm">
      <div className="max-h-[88vh] w-full max-w-2xl overflow-auto rounded-[28px] border border-border bg-surface p-5 shadow-2xl">
        ...
        {mode === 'evening' ? (
          <div className="mt-4 space-y-3">
            ...
            <textarea
              ...
              placeholder="哪里可以更好？"
              className="min-h-[88px] w-full rounded-2xl border border-border bg-background px-3 py-2 text-sm text-text outline-none focus:border-primary"
            />
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            ...
            <button type="button" className="inline-flex items-center gap-1 rounded-2xl border border-border px-3 py-2 text-sm font-semibold text-text" onClick={() => { onOpenQuickCapture(); onClose() }}>
              <ClipboardPlus size={14} /> 快速捕获
            </button>
            <button type="button" className="rounded-2xl border border-border px-3 py-2 text-sm font-semibold text-text" onClick={snoozeNonUrgent}>延后提醒 1 小时</button>
            <button type="button" className="rounded-2xl border border-border px-3 py-2 text-sm font-semibold text-text" onClick={() => { onNavigate('reminders'); onClose() }}>打开提醒中心</button>
            <button type="button" className="rounded-2xl border border-border px-3 py-2 text-sm font-semibold text-text" onClick={() => { onNavigate('habits'); onClose() }}>去打卡</button>
          </div>
        )}
      </div>
    </div>
