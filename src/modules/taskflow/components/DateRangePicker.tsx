  ...
  return (
    <div ref={rootRef} className="relative">
      <button
        ...
        className={`group flex min-w-[18rem] items-center gap-3 rounded-2xl border px-4 py-2.5 text-left shadow-sm transition-all ${
          hasRange
            ? 'border-blue-500/60 bg-blue-500/10 text-blue-700 shadow-blue-500/10 dark:text-blue-200'
            : 'border-gray-200 bg-white/70 text-gray-700 hover:border-blue-400/70 hover:bg-blue-50/70 dark:border-gray-800 dark:bg-white/[0.03] dark:text-gray-300 dark:hover:border-blue-500/60 dark:hover:bg-blue-500/10'
        }`}
        aria-expanded={open}
        aria-haspopup="dialog"
        aria-label="选择日期范围"
      >
        <span className={`grid h-9 w-9 place-items-center rounded-xl ${hasRange ? 'bg-blue-500 text-white' : 'bg-gray-100 text-gray-500 dark:bg-gray-800 dark:text-gray-300'}`}>
          <Icon name="calendar" className="w-4 h-4" />
        </span>
        <span className="min-w-0 flex-1">
          <span className="block text-[11px] font-medium uppercase tracking-[0.18em] text-gray-400 dark:text-gray-500">Due range</span>
      ...
      {open && (
        <div
          className="absolute left-0 top-full z-50 mt-3 w-[34rem] overflow-hidden rounded-3xl border border-gray-200 bg-white shadow-2xl shadow-black/15 dark:border-white/10 dark:bg-[#0d0d0f] dark:shadow-black/60"
          role="dialog"
          aria-label="日期范围日历"
        >
          <div className="flex items-stretch">
            <aside className="w-40 border-r border-gray-200 bg-gray-50/80 p-3 dark:border-white/10 dark:bg-white/[0.03]">
              <div className="mb-3 px-2 text-xs font-semibold text-gray-500 dark:text-gray-400">快速选择</div>
              <div className="space-y-1.5">
                {presets.map((preset) => (
                  ...
                  >
                    <span className="block text-sm font-semibold text-gray-900 dark:text-gray-100">{preset.label}</span>
                    <span className="block text-[11px] text-gray-500 dark:text-gray-400">{preset.sub}</span>
                  </button>
                ))}
              </div>
              <button
                type="button"
                onClick={clearRange}
                className="mt-4 w-full rounded-xl border border-gray-200 px-3 py-2 text-sm font-medium text-gray-500 transition hover:border-red-200 hover:bg-red-50 hover:text-red-600 dark:border-white/10 dark:hover:border-red-500/30 dark:hover:bg-red-500/10 dark:hover:text-red-300"
              >
                清除范围
              </button>
            </aside>
