      ...
      <div className="flex items-center gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="搜索归档任务..."
            aria-label="搜索归档任务"
            className="w-full rounded-xl border border-border bg-white pl-10 pr-4 py-2.5 text-sm focus:border-primary focus:outline-none"
          />
        </div>
        <span className="text-sm text-text-muted">{archivedTasks.length} 个归档任务</span>
      </div>
        ...
        <div className="space-y-2">
          {archivedTasks.map((task) => {
            ...
            return (
              <div
                key={task.id}
                className="group flex items-center gap-3 rounded-xl border border-border bg-white p-3 transition hover:border-primary/30"
              >
                ...
                <div className="min-w-0 flex-1 cursor-pointer" onClick={() => onEditTask(task)} role="button" tabIndex={0} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); onEditTask(task); } }} aria-label={`编辑任务: ${task.title}`}>
                  <div className="flex items-center gap-2">
                    ...
                    {cat && (
                      <span className="flex-shrink-0 rounded-full bg-surface-lighter px-2 py-0.5 text-[10px] text-text-muted">{cat.name}</span>
                    )}
                  </div>
                ...
                <button
                  ...
                  className="flex-shrink-0 rounded-lg border border-border px-2.5 py-1.5 text-xs text-text-muted transition hover:border-primary/30 hover:text-primary opacity-0 group-hover:opacity-100"
                  title="取消归档"
