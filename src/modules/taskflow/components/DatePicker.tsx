  ...
  return (
    <div ref={rootRef} className="relative" role="group" aria-label="日期选择器">
      <button
        ...
        className={`flex w-full items-center gap-3 rounded-2xl border px-4 py-3 text-left shadow-sm transition-all ${ value ? 'border-blue-500/60 bg-blue-500/10 text-blue-700 shadow-blue-500/10 dark:text-blue-200' : 'border-border bg-white/75 text-text hover:border-blue-400/70 hover:bg-blue-50/70 dark:bg-white/[0.03] dark:hover:border-blue-500/60 dark:hover:bg-blue-500/10' }`}
        aria-expanded={open}
        aria-haspopup="dialog"
      >
        <span className={`grid h-10 w-10 place-items-center rounded-xl ${value ? 'bg-blue-500 text-white' : 'bg-surface-lighter text-text-muted '}`}>
          <Icon name="calendar" className="w-4 h-4" />
        </span>
        <span className="min-w-0 flex-1">
          <span className="block text-[11px] font-medium uppercase tracking-[0.18em] text-text-muted">Due date</span>
      ...
      {open && (
        <div
          className="absolute left-0 top-full z-50 mt-3 w-[28rem] overflow-hidden rounded-3xl border border-border bg-white shadow-2xl shadow-black/15 dark:border-white/10 dark:bg-[#0d0d0f] dark:shadow-black/60"
          role="dialog"
          aria-label="选择截止日期"
        >
            ...
            <div className="grid grid-cols-5 gap-2">
              {presets.map((preset) => (
                <button
                  ...
                  className={`rounded-2xl px-3 py-2 text-left transition ${ value === preset.dateStr ? 'bg-blue-500 text-white shadow-lg shadow-blue-500/30' : 'bg-surface-lighter text-text hover:bg-blue-50 hover:text-blue-600 dark:bg-white/[0.04] dark:hover:bg-blue-500/10 dark:hover:text-blue-200' }`}
                >
                  <span className="block text-xs font-bold">{preset.label}</span>
                  <span className={`block text-[10px] ${value === preset.dateStr ? 'text-blue-100' : 'text-text-muted'}`}>{preset.sub}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="p-4">
            <div className="mb-4 flex items-center justify-between">
              <div>
                ...
                <div className="text-xs text-text-muted">{selectedDate ? formatDisplay(value) : '选择一个任务截止日'}</div>
              </div>
              ...
              {days.map((calendarDay) => {
                ...
                return (
                  <button
                    ...
                    className={`relative h-10 rounded-xl text-sm font-semibold transition-all ${ isSelected ? 'bg-blue-500 text-white shadow-lg shadow-blue-500/30' : calendarDay.inMonth ? 'text-text hover:bg-surface-lighter dark:hover:bg-white/10' : 'text-text-muted hover:bg-surface-lighter dark:text-text-muted dark:hover:bg-white/5' }`}
                  >
                    {calendarDay.day}
                    {isToday && !isSelected && (
          ...
          {value && (
            <button
              ...
              className="w-full border-t border-border px-4 py-3 text-sm font-semibold text-text-muted transition hover:bg-red-50 hover:text-red-600 dark:border-white/10 dark:hover:bg-red-500/10 dark:hover:text-red-300"
            >
              清除截止日期
            </button>
          )}
