...
const QUADRANTS: Array<{ id: Quadrant; label: string; desc: string; color: string }> = [
  ...
  { id: 'neither', label: '不紧急不重要', desc: '可以删除', color: 'border-gray-200 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-900/20' },
]
      ...
      {QUADRANTS.map((q) => (
        <div key={q.id} className={`rounded-2xl border-2 ${q.color} p-3 flex flex-col overflow-hidden`}>
          <div className="mb-2 flex items-center justify-between">
            <div>
              <h3 className="text-sm font-bold text-gray-700 dark:text-gray-300">{q.label}</h3>
              <p className="text-[10px] text-gray-400">{q.desc}</p>
            </div>
            <span className="rounded-full bg-white/60 dark:bg-gray-800/60 px-2 py-0.5 text-[10px] font-semibold text-gray-500">
              {quadrants[q.id].length}
            </span>
          </div>
          <div className="flex-1 space-y-1.5 overflow-y-auto">
            {quadrants[q.id].length === 0 ? (
              <div className="flex h-full items-center justify-center text-xs text-gray-300 dark:text-gray-600">暂无任务</div>
            ) : (
              quadrants[q.id].map((task) => {
                ...
                return (
                  <div
                    ...
                    className="cursor-pointer rounded-xl border border-gray-200/60 dark:border-gray-700/60 bg-white dark:bg-gray-800 p-2.5 transition hover:border-primary/30 hover:shadow-sm"
                  >
                    <div className="flex items-center gap-1.5">
                      <span className={`h-1.5 w-1.5 rounded-full ${PRIORITY_CONFIG[task.priority].color}`} />
                      <span className="truncate text-xs font-medium text-gray-700 dark:text-gray-300">{task.title}</span>
                    </div>
                    <div className="mt-1 flex items-center gap-1.5 text-[10px] text-gray-400">
                      {cat && <span className="rounded bg-gray-100 dark:bg-gray-700 px-1 py-0.5">{cat.name}</span>}
